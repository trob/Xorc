 ...
 * @deprecated Use instead Xorc\Model\DataBase\MySqli
...
class MySql {
	...
	public function __construct(){
		...
		$this->dbConnect = mysql_connect($this->registry['db']['server'], $this->registry['db']['user'], $this->registry['db']['password']);
		if (!$this->dbConnect){
			echo "Ошибка ".mysql_errno().": ".mysql_error();
			exit;
		}
		if(!mysql_select_db($this->registry['db']['db'])){
			echo "База данных $this->registry['db']['db'] отсутствует!";
			exit;
		}
	}
	...
	public function getObject($query, $type = 'object'){
		...
		if(is_resource($this->result) && mysql_num_rows($this->result)){
			switch ($type) {
				case 'object':
					$this->object = mysql_fetch_object($this->result);;
				break;
				case 'assoc':
					$this->object = mysql_fetch_assoc($this->result);
				break;
				case 'num':
					$this->object = mysql_fetch_array($this->result);
				break;
			}
